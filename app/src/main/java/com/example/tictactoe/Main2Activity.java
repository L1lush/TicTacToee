...
public class Main2Activity extends BaseGameActivity {
    private static final String DATABASE_URL =
            "https://tictactoe998899-default-rtdb.firebaseio.com/";
    ...
    private void setupFirebaseListener() {
        gameListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                // Check if the game was actually reset (e.g. all cells empty in snapshot)
                boolean firebaseReset = true;
                for (int r = 0; r < 3; r++) {
                    for (int c = 0; c < 3; c++) {
                ...
                for (int id : BUTTON_IDS) {
                    ...
                    String oldVal = model.getCell(r, c);

                    if (!cellVal.equals(oldVal)) {
                        model.forceMove(r, c, cellVal);
                        updateButton(button, cellVal);
                    } else if (cellVal.isEmpty()) {
                        // Ensure button is reset even if model already says empty (in case of UI sync)
                        button.setText("");
                        button.setScaleX(1f);
                        button.setScaleY(1f);
                        button.setAlpha(1f);
                        button.setBackgroundTintList(ContextCompat.getColorStateList(Main2Activity.this, R.color.cell_empty));
                    }
                }

                if ("O".equals(firebasePlayer)) {
                    if (!"O".equals(model.getCurrentPlayer())) model.changePlayer();
                } else if ("X".equals(firebasePlayer)) {
                    if (!"X".equals(model.getCurrentPlayer())) model.changePlayer();
                }
